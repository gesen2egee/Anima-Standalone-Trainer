...
*   `--knn_noise_k=K`: Selects the base training noise source. `K=0` keeps standard Gaussian sampling (default). `K>0` draws `K` Gaussian candidates per sample and picks the nearest one in flattened latent space.
*   `--use_cdc_fm`: Enables Carré du champ Flow Matching for ANIMA and KREA 2. It precomputes a low-rank local covariance from disk-cached latents, uses the exact probability path `x_sigma=(1-sigma)x+sigma*epsilon+(1-sigma)Gamma^(1/2)epsilon`, and trains against its exact velocity `epsilon-x-Gamma^(1/2)epsilon`. This requires both `--cache_latents` and `--cache_latents_to_disk`. Use `--cdc_combine_knn --knn_noise_k=K` to select an Immiscible Diffusion noise and feed that same noise through the CDC path in every step. `--cdc_knn_metric=l2` reproduces the paper's raw L2 selection; `mahalanobis` selects candidates with the stabilized local metric `(Gamma+tau*I)^-1`, where `--cdc_knn_regularization` sets tau relative to the largest local eigenvalue. Use `--cdc_switch_ratio=0.7` for 70% KNN-only steps followed by 30% CDC-only steps; add `--cdc_switch_reverse` to reverse this into 70% CDC-only followed by 30% KNN-only. Use `--cdc_alternate_knn` to alternate one KNN-only and one CDC-only optimizer step. Combined, switch, and alternate modes are mutually exclusive. The configured K value is always preserved when a CDC-only step does not use it. Tune the geometry with `--cdc_k_neighbors`, `--cdc_k_bandwidth`, `--cdc_dim`, `--cdc_gamma`, `--cdc_bandwidth_rescale`, and `--cdc_min_bucket_size`. Cached geometry is stored below `tasks/cdc_cache` by default.
*   `--use_self_flow`: Enables Self-Flow for `anima_train_network.py` and `krea2_train_network.py`. It samples two timesteps, applies the second timestep to a Bernoulli subset of image tokens, and aligns the student feature with an EMA-adapter teacher using cosine loss. It requires `--network_train_unet_only`, is mutually exclusive with CDC-FM and adapter dropout, and supports LoRA, LoHa, LoKr, KronA, and CDKA. Defaults follow the paper: `--self_flow_mask_ratio=0.25`, `--self_flow_representation_weight=0.8`, `--self_flow_ema_decay=0.9999`, `--self_flow_student_layer=8`, and `--self_flow_teacher_layer=20`. The projection head is training-only; inference checkpoints retain the selected adapter format and save EMA adapter weights by default.
*   `--immiscible_image_scale=S`: Scales training latents before diffusion for immiscible image scaling. `S=1.0` disables it. The improved immiscible diffusion paper discusses factors like `2` or `4`.
