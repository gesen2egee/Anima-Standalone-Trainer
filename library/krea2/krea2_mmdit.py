...
class Attention(torch.nn.Module):
    ...
    def forward(self, qkv: Tensor, freqs: Tensor | None = None, attn_params: AttentionParams | None = None) -> Tensor:
        ...
        # The shared local attention backend predates PyTorch's native GQA flag.
        # Expand K/V here so Krea2 keeps its 48Q/12KV checkpoint layout while
        # remaining compatible with the existing Anima attention implementation.
        if self.kvheads != self.heads:
            assert self.heads % self.kvheads == 0
...
class SingleStreamDiT(nn.Module):
    ...
    def enable_block_swap(self, num_blocks: int, device: torch.device):
        ...
        self.offloader = ModelOffloader(self.blocks, num_blocks, device, supports_backward=True)
        print(
            f"Krea 2: Block swap enabled. Swapping {num_blocks} of {num_main_blocks} blocks "
            f"to device {device}. Supports backward: {True}"
        )
