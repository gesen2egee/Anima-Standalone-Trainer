...
def add_anima_training_arguments(parser: argparse.ArgumentParser):
    ...
    parser.add_argument(
        ...
        type=str,
        default="sigmoid",
        choices=["sigma", "uniform", "sigmoid", "shift", "autoshift", "autoshift_wavelet", "flux_shift", "plora"],
...
def _sample_image_inference(
    ...
    def decode_sample_latents():
        try:
            return vae.decode_to_pixels(latents)
        except torch.OutOfMemoryError:
            logger.warning(
                "VAE sample decode OOM. Retrying with tiled decode, disabled VAE cache, and smaller spatial chunking."
            )
            clean_memory_on_device(accelerator.device)
            if hasattr(vae, "disable_cache"):
                vae.disable_cache()
            if hasattr(vae, "enable_spatial_chunking"):
                current_chunk = getattr(vae, "spatial_chunk_size", None)
                if current_chunk is None or current_chunk > 32:
                    vae.enable_spatial_chunking(32)
            ...
            try:
                ...
                logger.warning("VAE sample decode still OOM after fallback. Skipping this sample image.")
                clean_memory_on_device(accelerator.device)
                return None

    org_text_encoder_device = None
    if text_encoder is not None and sample_prompts_te_outputs is None:
        try:
            org_text_encoder_device = next(text_encoder.parameters()).device
        except StopIteration:
            org_text_encoder_device = None
        if org_text_encoder_device is not None and org_text_encoder_device.type != "cpu":
            logger.info("move text encoder to cpu during sample denoise/decode")
            text_encoder.to("cpu")
    ...
    try:
        ...
        org_vae_tiling = getattr(vae, "use_tiling", None)
        vae.to(accelerator.device)
        if hasattr(vae, "enable_tiling"):
            vae.enable_tiling()
        try:
            decoded = decode_sample_latents()
        finally:
            if org_vae_tiling is False and hasattr(vae, "disable_tiling"):
                vae.disable_tiling()
            vae.to(org_vae_device)
        clean_memory_on_device(accelerator.device)
    finally:
        if org_text_encoder_device is not None and org_text_encoder_device.type != "cpu":
            logger.info("move text encoder back to original device after sample")
            text_encoder.to(org_text_encoder_device)
            clean_memory_on_device(accelerator.device)
