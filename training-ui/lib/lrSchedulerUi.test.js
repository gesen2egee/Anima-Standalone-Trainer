...
test("LR scheduler mapping loads and saves warmup stable decay options", () => {
  ...
  assert.match(appJs, /\$\("cfg-lr-decay-steps"\)\.value = t\.lr_decay_steps \?\? 0/);
  ...
  assert.match(appJs, /lr_decay_steps:\s*\$\("cfg-lr-scheduler"\)\.value === "warmup_stable_decay"[\s\S]*?safeInt\(\$\("cfg-lr-decay-steps"\)\.value\)/);
});
